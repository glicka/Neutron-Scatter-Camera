...
//    const int CHUNKSIZE = 8;
    const int ECHUNKSIZE = 2048;
    const int RCHUNKSIZE = 8;
    ...
    hsize_t rchunkdims[2] = {RCHUNKSIZE, raw_data_length};
    //rchunkdims[0] = 10;//10
            ...
            do {  // loop over events
                    ...
                    if(rd3302 > 0){
                        ...
                        while(buffer_size > 0)  {
                                ...
                                hdf5event[0].detector = ID;//(gl_dma_rd_buffer[0]&0xffff);
                                hdf5event[0].ADC =  (gl_dma_rd_buffer[2] & 0x1000);//gl_dma_rd_buffer[ptr_index] - gl_dma_rd_buffer[ptr_index+1];
                                hdf5event[0].pileup = (gl_dma_rd_buffer[3] & 0xffff0000) << 24;
                                hdf5event[0].rid = nevents;
                                //unsigned int checksum = gl_dma_rd_buffer[ptr_index+3];
                                eventdataset.write(hdf5event,eventtype,eventdataspace,eventdataspace_temp);
