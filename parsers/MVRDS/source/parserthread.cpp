...
void parserthread::run(){
    ...
    const unsigned int TRIGGERBIT = pow(2.0, 24);
    const unsigned int PILEBIT = pow(2.0,30);
    const unsigned int RETRIGBIT = pow(2.0,31);
    //unsigned  int szip_options_mask;
    //unsigned  int szip_pixels_per_block;

    //LOAD CALIBRATION TABLE
    double mAry[NUM_DETECTORS];
    double bAry[NUM_DETECTORS];
    if( this->calMode ) { loadData1D( mAry, bAry, calFileName.toStdString() ); }

    FILE * gl_FILE_OfflineDataEvenFilePointer;

    // open data file
            gl_FILE_OfflineDataEvenFilePointer=fopen(filename.toAscii(),"rb");// filename to be opened
    ...
    hsize_t rchunkdims[2] = {CHUNKSIZE, raw_data_length};
    //rchunkdims[0] = 10;//10
            ...
            do {  // loop over events
                    ...
                    if(rd3302 > 0)
                    {
                        ...
                        for (ievent=0;ievent<events_in_buffer;ievent++)
                        {
                                ...
                                unsigned int Mod = ADCID >> 12;
                                if(Mod == 15){ Mod = 3; }
                                unsigned int ID = (((Mod-1) << 3) + (ADCID & 7));
//                                unsigned int Ch = (ADCID & 7); // 0 to 7
                                //unsigned int ID = ((Mod-1) << 3);
//                                unsigned int ID = ((Mod-1) << 3);    //ORIGINAL
//                                ID = ID + Ch - 56;

//                                cout << "DetID of event " << nevents << " = " << ID << endl;
                                if( ID < 0 || ID > NUM_DETECTORS ){ break; }
                                ...
                                if( this->calMode )
                                {
                                    ...
                                    //cout << "Energy of" << nevents+1 <<" = " << hdf5event[0].ADC << endl;
                                }
                                else{ hdf5event[0].ADC = gl_dma_rd_buffer[ptr_index]; }
//                                if (nevents == 1838830){ cout << "Got past energy setting" << endl; }
                                //NEW: Get the trigger, pileup, and retrigger flags
                                unsigned int bitline = gl_dma_rd_buffer[ptr_index+2];
//                                if (nevents == 1838830){ cout << "Got past reading bitline" << endl; }
...
//                                if (nevents == 1838830){ cout << "Wrote event #1838831" << endl; }
